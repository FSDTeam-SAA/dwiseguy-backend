...
const getCourseDetailsWithProgress = async (userId: string, courseId: string) => {
      ...
      const completionPercentage = totalSubLessons > 0 ? Math.round((completedCount / totalSubLessons) * 100) : 0;
...
const updateStudentProgress = async (userId: string, subLessonId: string) => {
    ...
    const nextSub = await SubLesson.findOne({
      ...
      await UserProgress.updateOne({ _id: progress._id }, { $addToSet: { completedLessons: currentLesson._id } });
